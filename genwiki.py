...
def get_org_tags(db, entity_id, org_name):
    ...
    # 1. Check for Wikipedia-sourced OR LLM classification fact
    classified = db.execute("""
        SELECT f.object FROM fact f
        WHERE f.entity_id = ? AND f.predicate = 'classified_as'
        ORDER BY CASE WHEN f.object IN ('think-tank','political-party','corporate','government-body','industry-association','ngo','org-generic') THEN 0 ELSE 1 END
        LIMIT 1
    """, [entity_id]).fetchone()
    
    if classified:
        category = classified[0].lower()
        if category in ['think-tank', 'political-party', 'corporate', 'government-body', 'industry-association', 'ngo']:
            return ['org', category]
