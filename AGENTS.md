...
- Data from CSVs whose provenance is unknown (who assembled them, when, from what source)
- Inferences or keyword matches that cannot be pinned to a specific sentence in a source document
- Secondary datasets without documented extraction methodology

**Never ingest data from a CSV without knowing its provenance.** If a CSV was generated by a script, that script must be committed and the source query must be reproducible. If the CSV was manually assembled, the sources and methodology must be documented.
