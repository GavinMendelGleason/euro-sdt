---
id: corp-oliver-zipse
title: Oliver Zipse
type: person
category: corporate_elite
tags:
  - person
  - corporate
  - economic-elite
generated: 2026-05-11
---

# Oliver Zipse


*← [Bodies](../bodies.md)*

## Educated at

- **University Of Deusto**
  > "He studied computer science and mathematics at the University of Utah in Salt Lake City from 1983 to 1985 without obtaining a degree."
  > 📎 [Wikipedia: Oliver Zipse](https://en.wikipedia.org/wiki/Oliver_Zipse)

- **Technische Universität Darmstadt**
  > "In 1985 he switched to mechanical engineering at the Technische Universität Darmstadt and graduated in 1991 with a degree in mechanical engineering."
  > 📎 [Wikipedia: Oliver Zipse](https://en.wikipedia.org/wiki/Oliver_Zipse)

- **Kellogg-WHU Executive MBA Program (Kellogg School of Management, US**

- **WHU – Otto Beisheim School of Management, Germany)**


## Organisation affiliations

- **Baden-Badener Unternehmer-Gespräche (BBUG)** — Member of the Board
  > "&#91; 12 &#93; &#91; 13 &#93; &#91; 14 &#93; 
 
 Zipse visit Chongqing , China , 2024 
 In his capacity as CEO, Zipse accompanied Chancellor Olaf Scholz on a state visit to China in 2024."
  > 📎 [Wikipedia: Oliver Zipse](https://en.wikipedia.org/wiki/Oliver_Zipse)

- **European Automobile Manufacturers Association (ACEA)** — Member of the Board of Directors
  > "&#91; 15 &#93; 
 
 Other activities [ edit ] 
 Baden-Badener Unternehmer-Gespräche (BBUG), Member of the Board (since 2019) &#91; 16 &#93; 
 European Automobile Manufacturers Association (ACEA), Membe"
  > 📎 [Wikipedia: Oliver Zipse](https://en.wikipedia.org/wiki/Oliver_Zipse)

- **European Round Table of Industrialists (ERT)** — Member
  > "&#91; 21 &#93; He supports BMW's expansions towards e-mobility production capacities, stating: "In the future, every BMW Group plant in Europe will be equipped to produce electrified as well as conven"
  > 📎 [Wikipedia: Oliver Zipse](https://en.wikipedia.org/wiki/Oliver_Zipse)

- **Fraunhofer Society** — Member of the Senate
  > "&#91; 23 &#93; 
 Zipse is an advocate of experimenting with and pioneering new technologies within the BMW group."
  > 📎 [Wikipedia: Oliver Zipse](https://en.wikipedia.org/wiki/Oliver_Zipse)

- **German Association of the Automotive Industry (VDA)** — Member of the Managing Board
  > "Supporting the use of exoskeleton vests for workers at BMW's plant in Spartanburg , he stated: "We take out all opportunities to try something new, which we haven't done so far."
  > 📎 [Wikipedia: Oliver Zipse](https://en.wikipedia.org/wiki/Oliver_Zipse)

- **BMW** — board member
  > "Extracted from Wikipedia page for BMW"
  > 📎 [Wikipedia: BMW](https://en.wikipedia.org/wiki/BMW)


### Education clusters