---
id: mario-monti
title: Mario Monti
type: person
category: commissioner
country: ITA
tags:
  - person
  - commissioner
  - political-elite
generated: 2026-05-08
---

# Mario Monti

**Commissioner** · [Italy](../countries/ita.md)

*← [Bodies](../bodies.md)*

## Served on

- **Santer Commission**
  > "Name "Mario Monti" appears in Wikipedia page for commission-santer"
  > 📎 [Wikipedia article: Mario Monti](https://en.wikipedia.org/wiki/Mario_Monti)
  > Source: sources/wikipedia/commission-santer.txt (11541 chars)

- **Prodi Commission**
  > "As Competition Commissioner, Monti led the investigation into a number of high-profile and controversial mergers, including Scania AB and Volvo (1999), WorldCom and Sprint (2000), General Electric and"
  > 📎 [Wikipedia article: Mario Monti](https://en.wikipedia.org/wiki/Mario_Monti)
  > Name "Mario Monti" found in Wikipedia table extract for commission-prodi


## Portfolio

- **Internal Market and Financial Services / Competition**
  > "Monti served as a European Commissioner from 1995 to 2004, with responsibility for the Internal Market, Services, Customs and Taxation from 1995 to 1999 and for Competition from 1999 to 2004."
  > 📎 [Wikipedia article: Mario Monti](https://en.wikipedia.org/wiki/Mario_Monti)
  > Mario Monti  (Italian: [ˈmaːrjo ˈmonti]; born 19 March 1943) is an Italian politician, economist and academic who served as the prime minister of Ital

- **Competition**
  > "Monti served as a European Commissioner from 1995 to 2004, with responsibility for the Internal Market, Services, Customs and Taxation from 1995 to 1999 and for Competition from 1999 to 2004."
  > 📎 [Wikipedia article: Mario Monti](https://en.wikipedia.org/wiki/Mario_Monti)
  > Mario Monti  (Italian: [ˈmaːrjo ˈmonti]; born 19 March 1943) is an Italian politician, economist and academic who served as the prime minister of Ital


## Nominated by

- **ITA**
  > "Mario Monti present in wikipedia/mario-monti.txt — verifies nominated_by: ITA"
  > 📎 *Organisations classified dataset (manually researched)*
  > Primary source: wikipedia/mario-monti.txt. Context: Mario Monti  (Italian: [ˈmaːrjo ˈmonti]; born 19 March 1943) is an Italian politician, economist a

- **ITA**
  > "Mario Monti present in wikipedia/mario-monti.txt — verifies nominated_by: ITA"
  > 📎 *Organisations classified dataset (manually researched)*
  > Primary source: wikipedia/mario-monti.txt. Context: Mario Monti  (Italian: [ˈmaːrjo ˈmonti]; born 19 March 1943) is an Italian politician, economist a


## Country

- **Italy**
  > "Mario Monti  (Italian: [ˈmaːrjo ˈmonti]; born 19 March 1943) is an Italian politician, economist and academic who served as the prime minister of Italy from 2011 to 2013, leading a technocratic govern"
  > 📎 [Wikipedia article: Mario Monti](https://en.wikipedia.org/wiki/Mario_Monti)
  > LLM re-match (phrase 0)

- **Italy**
  > "Mario Monti  (Italian: [ˈmaːrjo ˈmonti]; born 19 March 1943) is an Italian politician, economist and academic who served as the prime minister of Italy from 2011 to 2013, leading a technocratic govern"
  > 📎 [Wikipedia article: Mario Monti](https://en.wikipedia.org/wiki/Mario_Monti)
  > LLM re-match (phrase 0)


## Educated at

- **Bocconi**
  > "Monti studied at the private Leo XIII High School and attended Bocconi University of Milan, where he obtained a degree in economics in 1965."
  > 📎 [Wikipedia article: Mario Monti](https://en.wikipedia.org/wiki/Mario_Monti)
  > LLM re-match (phrase 10)

- **Charles University**
  > "Later, he won a scholarship to Yale University, where he studied under James Tobin, a recipient of the Nobel Prize in Economics."
  > 📎 [Wikipedia article: Mario Monti](https://en.wikipedia.org/wiki/Mario_Monti)
  > Mario Monti  (Italian: [ˈmaːrjo ˈmonti]; born 19 March 1943) is an Italian politician, economist and academic who served as the prime minister of Ital

- **Leo Xiii High School**
  > "Monti studied at the private Leo XIII High School and attended Bocconi University of Milan, where he obtained a degree in economics in 1965."
  > 📎 [Wikipedia article: Mario Monti](https://en.wikipedia.org/wiki/Mario_Monti)
  > Wikipedia extract for Mario Monti

- **Bocconi**
  > "Monti studied at the private Leo XIII High School and attended Bocconi University of Milan, where he obtained a degree in economics in 1965."
  > 📎 [Wikipedia article: Mario Monti](https://en.wikipedia.org/wiki/Mario_Monti)
  > Wikipedia extract for Mario Monti

- **Charles University**
  > "Later, he won a scholarship to Yale University, where he studied under James Tobin, a recipient of the Nobel Prize in Economics."
  > 📎 [Wikipedia article: Mario Monti](https://en.wikipedia.org/wiki/Mario_Monti)
  > Wikipedia extract for Mario Monti


## Organisation affiliations

- **suerf-the-european-money-and-finance-forum** — President
  > "He also served as President of the SUERF (The European Money and Finance Forum) from 1982 to 1985."
  > 📎 [Wikipedia article: Mario Monti](https://en.wikipedia.org/wiki/Mario_Monti)
  > Source: `sources/wikipedia/mario-monti.txt` (14557 chars)

- **bruegel** — First President
  > "Other activities
Monti actively participates in several major think tanks."
  > 📎 [Wikipedia article: Mario Monti](https://en.wikipedia.org/wiki/Mario_Monti)
  > Source: `sources/wikipedia/mario-monti.txt` (14557 chars)

- **Bilderberg Group** — Leading member
  > "Monti is also a leading member of the exclusive Bilderberg Group."
  > 📎 [Wikipedia article: Mario Monti](https://en.wikipedia.org/wiki/Mario_Monti)
  > Source: `sources/wikipedia/mario-monti.txt` (14557 chars)

- **Trilateral Commission** — European Chairman
  > "In 2010, he became President of the Trilateral Commission."
  > 📎 [Wikipedia article: Mario Monti](https://en.wikipedia.org/wiki/Mario_Monti)
  > Source: `sources/wikipedia/mario-monti.txt` (14557 chars)

- **tats-g-n-raux-de-l-europe** — First supporter
  > "In 2007, Monti was one of the first supporters of the first European civic forum, États Généraux de l'Europe, initiated by European think tank EuropaNova and European Movement."
  > 📎 [Wikipedia article: Mario Monti](https://en.wikipedia.org/wiki/Mario_Monti)
  > Source: `sources/wikipedia/mario-monti.txt` (14557 chars)

- **europanova** — Supporter (via think tank)
  > "In 2007, Monti was one of the first supporters of the first European civic forum, États Généraux de l'Europe, initiated by European think tank EuropaNova and European Movement."
  > 📎 [Wikipedia article: Mario Monti](https://en.wikipedia.org/wiki/Mario_Monti)
  > Source: `sources/wikipedia/mario-monti.txt` (14557 chars)

- **European Movement** — Supporter (via organization)
  > "In 2007, Monti was one of the first supporters of the first European civic forum, États Généraux de l'Europe, initiated by European think tank EuropaNova and European Movement."
  > 📎 [Wikipedia article: Mario Monti](https://en.wikipedia.org/wiki/Mario_Monti)
  > Source: `sources/wikipedia/mario-monti.txt` (14557 chars)

- **attali-commission** — Member
  > "He was also a member of the French government's Attali Commission from 2007 to 2008, appointed by Nicolas Sarkozy to provide recommendations to enhance economic growth in France."
  > 📎 [Wikipedia article: Mario Monti](https://en.wikipedia.org/wiki/Mario_Monti)
  > Source: `sources/wikipedia/mario-monti.txt` (14557 chars)

- **Spinelli Group** — Founding member
  > "Monti is a founding member of the Spinelli Group, an organization launched in September 2010 to facilitate integration within the European Union (other members of the steering group include Jacques De"
  > 📎 [Wikipedia article: Mario Monti](https://en.wikipedia.org/wiki/Mario_Monti)
  > Source: `sources/wikipedia/mario-monti.txt` (14557 chars)

- **high-level-group-on-own-resources** — Chairman
  > "In January 2014, Monti became Chairman of the High Level Group on Own Resources, a consultative committee of the European Union aimed at proposing new forms of revenue for the European Union's budget."
  > 📎 [Wikipedia article: Mario Monti](https://en.wikipedia.org/wiki/Mario_Monti)
  > Source: `sources/wikipedia/mario-monti.txt` (14557 chars)

- **center-for-economic-and-policy-research-cepr** — Distinguished Fellow
  > "Other activities include: 

Center for Economic and Policy Research (CEPR), Distinguished Fellow (since 2019)
Centre for European Reform (CER), Member of the Advisory Board
European Investment Bank (E"
  > 📎 [Wikipedia article: Mario Monti](https://en.wikipedia.org/wiki/Mario_Monti)
  > Source: `sources/wikipedia/mario-monti.txt` (14557 chars)

- **centre-for-european-reform** — Member of the Advisory Board
  > "Other activities include: 

Center for Economic and Policy Research (CEPR), Distinguished Fellow (since 2019)
Centre for European Reform (CER), Member of the Advisory Board
European Investment Bank (E"
  > 📎 [Wikipedia article: Mario Monti](https://en.wikipedia.org/wiki/Mario_Monti)
  > Source: `sources/wikipedia/mario-monti.txt` (14557 chars)

- **european-investment-bank** — Member of the Appointment Advisory Committee
  > "Other activities include: 

Center for Economic and Policy Research (CEPR), Distinguished Fellow (since 2019)
Centre for European Reform (CER), Member of the Advisory Board
European Investment Bank (E"
  > 📎 [Wikipedia article: Mario Monti](https://en.wikipedia.org/wiki/Mario_Monti)
  > Source: `sources/wikipedia/mario-monti.txt` (14557 chars)

- **Friends of Europe** — Member of the Praesidium
  > "Other activities include: 

Center for Economic and Policy Research (CEPR), Distinguished Fellow (since 2019)
Centre for European Reform (CER), Member of the Advisory Board
European Investment Bank (E"
  > 📎 [Wikipedia article: Mario Monti](https://en.wikipedia.org/wiki/Mario_Monti)
  > Source: `sources/wikipedia/mario-monti.txt` (14557 chars)

- **moody-s** — Member of the Senior European Advisory Council
  > "Other activities include: 

Center for Economic and Policy Research (CEPR), Distinguished Fellow (since 2019)
Centre for European Reform (CER), Member of the Advisory Board
European Investment Bank (E"
  > 📎 [Wikipedia article: Mario Monti](https://en.wikipedia.org/wiki/Mario_Monti)
  > Source: `sources/wikipedia/mario-monti.txt` (14557 chars)

- **Atlantic Council** — Member of the Business and Economics Advisors Group
  > "Other activities include: 

Center for Economic and Policy Research (CEPR), Distinguished Fellow (since 2019)
Centre for European Reform (CER), Member of the Advisory Board
European Investment Bank (E"
  > 📎 [Wikipedia article: Mario Monti](https://en.wikipedia.org/wiki/Mario_Monti)
  > Source: `sources/wikipedia/mario-monti.txt` (14557 chars)

- **transparency-international** — Chairman of the European Advisory Board
  > "Other activities include: 

Center for Economic and Policy Research (CEPR), Distinguished Fellow (since 2019)
Centre for European Reform (CER), Member of the Advisory Board
European Investment Bank (E"
  > 📎 [Wikipedia article: Mario Monti](https://en.wikipedia.org/wiki/Mario_Monti)
  > Source: `sources/wikipedia/mario-monti.txt` (14557 chars)


## Member of

- **Bilderberg Group** — Bilderberg
  > "Mario Monti present in wikipedia/mario-monti.txt — verifies member_of: bilderberg"
  > 📎 *Organisations classified dataset (manually researched)*
  > Primary source: wikipedia/mario-monti.txt. Context: Mario Monti  (Italian: [ˈmaːrjo ˈmonti]; born 19 March 1943) is an Italian politician, economist a

- **Bilderberg Group** — Bilderberg
  > "Mario Monti present in wikipedia/mario-monti.txt — verifies member_of: bilderberg"
  > 📎 *Organisations classified dataset (manually researched)*
  > Primary source: wikipedia/mario-monti.txt. Context: Mario Monti  (Italian: [ˈmaːrjo ˈmonti]; born 19 March 1943) is an Italian politician, economist a

- **Trilateral Commission**
  > "In 2010, he became President of the Trilateral Commission."
  > 📎 [Wikipedia article: Mario Monti](https://en.wikipedia.org/wiki/Mario_Monti)
  > Mario Monti  (Italian: [ˈmaːrjo ˈmonti]; born 19 March 1943) is an Italian politician, economist and academic who served as the prime minister of Ital

- **Trilateral Commission**
  > "In 2010, he became President of the Trilateral Commission."
  > 📎 [Wikipedia article: Mario Monti](https://en.wikipedia.org/wiki/Mario_Monti)
  > Mario Monti  (Italian: [ˈmaːrjo ˈmonti]; born 19 March 1943) is an Italian politician, economist and academic who served as the prime minister of Ital


### Education clusters