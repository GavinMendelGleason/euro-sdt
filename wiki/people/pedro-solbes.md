---
id: pedro-solbes
title: Pedro Solbes
type: person
category: commissioner
country: ESP
tags:
  - person
  - commissioner
  - political-elite
generated: 2026-05-11
---

# Pedro Solbes

**Commissioner** · [Spain](../countries/esp.md)

*← [Bodies](../bodies.md)*

## Portfolio

- **Economic and Monetary Affairs**
  > "Appointed by Spanish premier José María Aznar (of the main competing party, the People's Party), Solbes was a European commissioner for economic and monetary affairs in the European Commission preside"
  > 📎 [Wikipedia article: Pedro Solbes](https://en.wikipedia.org/wiki/Pedro_Solbes)
  > Pedro Solbes Mira (31 August 1942 – 18 March 2023) was a Spanish economist. He was the president of FRIDE, Madrid-based think tank.


Life and career



## Nominated by

- **ESP**
  > "Pedro Solbes present in wikipedia/pedro-solbes.txt — verifies nominated_by: ESP"
  > 📎 *Organisations classified dataset (manually researched)*
  > Primary source: wikipedia/pedro-solbes.txt. Context: Pedro Solbes Mira (31 August 1942 – 18 March 2023) was a Spanish economist. He was the president 


## Country

- **Spain**
  > "While independent in the sense of not being affiliated to any party, his various ministerial roles in Spain were always within Socialist Workers' Party cabinets."
  > 📎 [Wikipedia article: Pedro Solbes](https://en.wikipedia.org/wiki/Pedro_Solbes)
  > Pedro Solbes Mira (31 August 1942 – 18 March 2023) was a Spanish economist. He was the president of FRIDE, Madrid-based think tank.


Life and career



## Educated at

- **Complutense University Of Madrid**
  > "Wikidata P69: Complutense University of Madrid"
  > 📎 [Wikipedia article: Pedro Solbes](https://en.wikipedia.org/wiki/Pedro_Solbes)
  > Wikidata educated-at for Pedro Solbes

- **Université Libre De Bruxelles**
  > "Wikidata P69: Free University of Brussels"
  > 📎 [Wikipedia article: Pedro Solbes](https://en.wikipedia.org/wiki/Pedro_Solbes)
  > Wikidata educated-at for Pedro Solbes


## Organisation affiliations

- **fride** — president
  > "He was the president of FRIDE, Madrid-based think tank."
  > 📎 [Wikipedia article: Pedro Solbes](https://en.wikipedia.org/wiki/Pedro_Solbes)
  > Source: `sources/wikipedia/pedro-solbes.txt` (1940 chars)

- **african-development-bank** — ex-officio member of the Board of Governors
  > "Other activities
African Development Bank (AfDB), ex-officio member of the Board of Governors (2004–2009)
Asian Development Bank (ADB), ex-officio member of the Board of Governors (2004–2009)
European"
  > 📎 [Wikipedia article: Pedro Solbes](https://en.wikipedia.org/wiki/Pedro_Solbes)
  > Source: `sources/wikipedia/pedro-solbes.txt` (1940 chars)

- **asian-development-bank-adb** — ex-officio member of the Board of Governors
  > "Other activities
African Development Bank (AfDB), ex-officio member of the Board of Governors (2004–2009)
Asian Development Bank (ADB), ex-officio member of the Board of Governors (2004–2009)
European"
  > 📎 [Wikipedia article: Pedro Solbes](https://en.wikipedia.org/wiki/Pedro_Solbes)
  > Source: `sources/wikipedia/pedro-solbes.txt` (1940 chars)

- **European Bank for Reconstruction and Development (EBRD)** — ex-officio member of the Board of Governors
  > "Other activities
African Development Bank (AfDB), ex-officio member of the Board of Governors (2004–2009)
Asian Development Bank (ADB), ex-officio member of the Board of Governors (2004–2009)
European"
  > 📎 [Wikipedia article: Pedro Solbes](https://en.wikipedia.org/wiki/Pedro_Solbes)
  > Source: `sources/wikipedia/pedro-solbes.txt` (1940 chars)

- **european-investment-bank** — ex-officio member of the Board of Governors
  > "Other activities
African Development Bank (AfDB), ex-officio member of the Board of Governors (2004–2009)
Asian Development Bank (ADB), ex-officio member of the Board of Governors (2004–2009)
European"
  > 📎 [Wikipedia article: Pedro Solbes](https://en.wikipedia.org/wiki/Pedro_Solbes)
  > Source: `sources/wikipedia/pedro-solbes.txt` (1940 chars)


### Education clusters